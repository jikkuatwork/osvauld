  ...
  "node_modules/simple-peer/index.js"(exports, module) {
    ...
    var Peer = class _Peer extends stream.Duplex {
      ...
      _onChannelMessage(event) {
        if (this.destroyed) return;
        let data = event.data;
        if (data instanceof ArrayBuffer) data = Buffer3.from(data);
        this.push(data);
      }
...
var MessageType = /* @__PURE__ */ ((MessageType2) => {
  MessageType2["SYNC_REQUEST"] = "sync_request";
  MessageType2["SYNC_RESPONSE"] = "sync_response";
  MessageType2["UPDATE"] = "update";
  MessageType2["OFFER"] = "offer";
  MessageType2["ANSWER"] = "answer";
  MessageType2["ICE_CANDIDATE"] = "ice_candidate";
...
var WebRTCPeer = class extends import_events.EventEmitter {
  id;
  state;
  peer = null;
  config;
  ...
  constructor(config) {
    super();
    this.id = config.peerId || v4_default();
  ...
  async connect() {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.peer = new import_simple_peer.default(peerOptions);
   ...
   */
  async send(message) {
    if (this.state !== "connected" /* CONNECTED */ || !this.peer) {
   ...
   */
  isConnected() {
    return this.state === "connected" /* CONNECTED */ && this.peer !== null;
  }
  /**
...
function createInitiator(config) {
...
async function connectLocalPeers(peer1, peer2) {
...
function generateShareCode(peerId, signal, timestamp) {
  const shareCode = {
    version: 1,
    peerId,
    signal,
...
async function generateQRCode(shareCode, options) {
  const opts = { ...DEFAULT_OPTIONS, ...options };
  try {
    const qrCodeDataUrl = await import_qrcode.default.toDataURL(shareCode, {
      width: opts.qrWidth,
...
async function generateQRCodeBuffer(shareCode, options) {
  const opts = { ...DEFAULT_OPTIONS, ...options };
  try {
    const buffer = await import_qrcode.default.toBuffer(shareCode, {
      width: opts.qrWidth,
      errorCorrectionLevel: opts.errorCorrectionLevel,
      margin: 2
    });
...
var SignalExchangeManager = class {
  signals = /* @__PURE__ */ new Map();
  /**
   * Stores a signal for a peer
   */
  storeSignal(peerId, signal) {
   ...
   */
  getLatestSignal(peerId) {
    const signals = this.signals.get(peerId);
    return signals && signals.length > 0 ? signals[signals.length - 1] : void 0;
  }
  /**
