  ...
  program
    ...
    .action(
      async (
        documentId: string,
        recipientKey: string,
        options: { capabilities: string; expires: string }
      ) => {
        ...
        try {
          ...
          for (const cap of capStrings) {
            if (cap === 'read') capabilities.push(Capability.READ);
            else if (cap === 'write') capabilities.push(Capability.WRITE);
            else if (cap === 'share') capabilities.push(Capability.SHARE);
            else if (cap === 'delete') capabilities.push(Capability.DELETE);
            else {
              spin.stop();
 ...
 * List shares command
 */
export function listSharesCommand(program: Command): void {
  program
    .command('shares')
 ...
 */
export function revokeShareCommand(program: Command): void {
  program
    .command('revoke')
    .description('Revoke a share link')
    .argument('<shareId>', 'Share ID')
    .option('-y, --yes', 'Skip confirmation')
    .action(async (shareId: string, options: { yes?: boolean }) => {
      if (!options.yes) {
 ...
 */
export function verifyAccessCommand(program: Command): void {
  program
    .command('verify')
    ...
    .action(async (token: string, documentId: string, capabilityStr: string) => {
      try {
        const app = getAuthenticatedApp();

        let capability: Capability;
        if (capabilityStr === 'read') capability = Capability.READ;
        else if (capabilityStr === 'write') capability = Capability.WRITE;
        else if (capabilityStr === 'share') capability = Capability.SHARE;
        else if (capabilityStr === 'delete') capability = Capability.DELETE;
        else {
